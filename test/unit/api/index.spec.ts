...
import { reqUserList, API_GET_USERS } from '../../../api';
...
describe('reqUserList', () => {
    it('reqUserList returns the user list on successful request', async () => {
        ...
        fetch.mockResolvedValue(createFetchResponse(userListResponse))

        const userList = await reqUserList()

        expect(fetch).toHaveBeenCalledWith(API_GET_USERS)

        expect(userList).toStrictEqual(userListResponse)
    });

    it('reqUserList throws an error on failed request', async () => {
